...
module.exports = bundler => {
    ...
    bundler.on('bundled', bundle => {
        ...
        if (config.swDest) {
            config.swDest = config.useRelativeDest
                ? config.swDest
                : path.resolve(outDir, config.swDest);
        }
        ...
        if (config.usePrecacheRoutes) {
            if (!fs.existsSync(config.swDest || swDest)) {
                fs.writeFileSync(config.swDest || swDest);
            }
        }
        ...
        delete swConfig.usePrecacheRoutes;
